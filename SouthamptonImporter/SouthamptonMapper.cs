...
public class SouthamptonMapper
{
    ...
    public SouthamptonMapper(ISouthamptonClientService southamptonClientService, IOrganisationClientService organisationClientService, string adminAreaCode)
    {
...
#pragma warning disable S1075 // URIs should not be hardcoded
        var southamptonCouncil = new OrganisationWithServicesDto(
        "40371b83-7465-4314-822b-9917e109a125",
            new OrganisationTypeDto("1", "LA", "Local Authority"), "Southampton City Council", "Southampton City Council", null, new Uri("https://www.southampton.gov.uk/").ToString(), "https://www.southampton.gov.uk/", new List<ServiceDto>(), new List<LinkContactDto>());

        southamptonCouncil.AdminAreaCode = _adminAreaCode;

        await _organisationClientService.CreateOrganisation(southamptonCouncil);

        return southamptonCouncil;
#pragma warning restore S1075 // URIs should not be hardcoded
    ...
    public async Task AddOrUpdateServices()
    {
        ...
        try
        {
            SouthamptonSimpleService southamptonSimpleService = await _southamptonClientService.GetServicesByPage(startPage);
            int totalPages = southamptonSimpleService.totalPages;
            foreach (Content content in southamptonSimpleService.content)
            {
            ...
            {
                southamptonSimpleService = await _southamptonClientService.GetServicesByPage(i);
    ...
    private async Task<int> AddAndUpdateService(Content content)
    {
        ...
        var newService = builder.WithMainProperties(
            ...
            assuredDate: DateTime.SpecifyKind(content.assured_date, DateTimeKind.Utc),
            attendingAccess: null, //content.attending_access,
            attendingType: null, //content.attending_type,
            deliverableType: null, //content.deliverable_type,
            status: content.status,
            fees: null, //content.fees,
    ...
    private List<EligibilityDto> GetEligibilityDtos(Eligibility[] eligibilities, ServiceDto existingService)
    {
        ...
        foreach (Eligibility eligibility in eligibilities)
        {
            ...
            int maxAge = eligibility.maximum_age ?? 0;
            int minAge = eligibility.minimum_age ?? 0;

            listEligibilityDto.Add(new EligibilityDto(id: eligibilityId, eligibilityDescription: eligibility.eligibility ?? eligibility.id, maximumAge: maxAge, minimumAge: minAge));
        }
    ...
    private List<CostOptionDto> GetCostOptionDtos(string serviceId, CostOptions[] costOptions, ServiceDto existingService)
    {
        ...
        foreach (CostOptions costOption in costOptions)
        {
            ...
            listCostOptionDto.Add(new CostOptionDto(id: costOptionId, amountDescription: costOption.amount_description, amount: costOption.amount, linkId: serviceId, option: costOption.option, validFrom: null, validTo: null));
        }
    ...
    private List<ServiceAreaDto> GetServiceAreas(ServiceAreas[] serviceAreas, ServiceDto existingService)
    {
        ...
        foreach (ServiceAreas serviceArea in serviceAreas)
        {
            ...
            listServiceAreaDto.Add(new ServiceAreaDto(id: serviceAreaId, serviceAreaDescription: serviceArea.name, extent: null, uri: null));
        }
    ...
    */
    ...
    private List<RegularScheduleDto> GetRegularSchedules(RegularSchedule[] regularSchedules, ServiceDto existingService)
    {
        ...
        foreach (RegularSchedule regularSchedule in regularSchedules)
        {
            ...
            listRegularScheduleDto.Add(new RegularScheduleDto(id: regularScheduleId, description: regularSchedule.weekday, opensAt: GetDateFromString(regularSchedule.opens_at), closesAt: GetDateFromString(regularSchedule.closes_at), byDay: null, byMonthDay: null, dtStart: null, freq: null, interval: null, validFrom: null, null));
        }
    ...
    */
    private List<ServiceAtLocationDto> GetServiceAtLocations(ServiceAtLocations[] serviceAtLocations, ServiceDto existingService)
    {
        ...
        foreach (ServiceAtLocations serviceAtLocation in serviceAtLocations)
        {
            ...
            if (serviceAtLocation == null || serviceAtLocation.location.latitude == null)
            {
                continue;
            }
            ...
            {
                if (string.IsNullOrEmpty(physicalAddress.address1) || string.IsNullOrEmpty(physicalAddress.postal_code))
                    continue;

                string physicalAddressId = $"{_adminAreaCode.Replace("E", "")}{serviceAtLocation.location.id}{physicalAddress.address1}{physicalAddress.postal_code}";
                physicalAddressId = physicalAddressId.Replace(",", "").Trim();
                physicalAddressId = physicalAddressId.Replace(" ", "-");
                physicalAddressDtos.Add(new PhysicalAddressDto(id: physicalAddressId, address1: physicalAddress.address1, city: physicalAddress.city, postCode: physicalAddress.postal_code, country: physicalAddress.country, stateProvince: physicalAddress.state_province));

            }
            LocationDto location = new(id: Id, name: !string.IsNullOrEmpty(serviceAtLocation.location.name) ? serviceAtLocation.location.name : Id, description: !string.IsNullOrEmpty(serviceAtLocation.location.name) ? serviceAtLocation.location.name : Id, latitude: latitude, longitude: longitude, physicalAddresses: physicalAddressDtos, linkTaxonomies: new List<LinkTaxonomyDto>(), linkContacts: new List<LinkContactDto>());

            listServiceAtLocationDto.Add(new ServiceAtLocationDto(id: Id,
                location: location,
                regularSchedules: new List<RegularScheduleDto>(), //regularSchedules,
                holidaySchedules: new List<HolidayScheduleDto>(), //holidaySchedules,
                linkContacts: new List<LinkContactDto>()));
        }
    ...
    private List<LanguageDto> GetLanguages(Language[] languages, ServiceDto existingService)
    {
        ...
        foreach (Language language in languages)
        {
            ...
            listLanguageDto.Add(new LanguageDto(id: serviceAreaId, name: language.name));
        }
    ...
    private async Task CreateOrganisationDictionary()
    {
        ...
        var southampton = organisations.FirstOrDefault(x => x.Name.Contains("Southampton"));
        if (southampton == null)
        {
